...
function About_home(dataabouthome) {
  let List_about_Home = document.querySelector(".about_render_video");
  if (List_about_Home) {
    List_about_Home.innerHTML = "";
    for (let item of dataabouthome) {
      var Comments_posts = "";
      for (let itemcmtposts of comments) {
          Comments_posts += `
            <div class="info_comment_video_page">
                <img src="${itemcmtposts.avatar_comment}" alt="">
                <div class="text_logo_name_videos">
                <p>${itemcmtposts.name_user_comment}<i class='bx bxs-check-circle' style='color:#2e88ff'></i></p>
            <p>${itemcmtposts.time_date} ~ <i class='bx bx-world'></i></p>
        </div>
        </div>
            <div class="value_comment_user">
                <p>
                    ${itemcmtposts.content}
                </p>
            </div>
        `
      }
