...
function About_home(dataabouthome) {
  ...
  if (ListaboutHome) {
    ...
    for (let item of dataabouthome) {
         var Comments_posts = "";
        for (let _comment of item.comments) {
          Comments_posts += `
            <div class="info_comment_video_page">
                <img src="${_comment.avatar_comment}" alt="">
                <div class="text_logo_name_videos">
                <p>${_comment.name_user_comment}<i class='bx bxs-check-circle' style='color:#2e88ff'></i></p>
            <p>${_comment.time_date} ~ <i class='bx bx-world'></i></p>
        </div>
        </div>
            <div class="value_comment_user">
                <p>
