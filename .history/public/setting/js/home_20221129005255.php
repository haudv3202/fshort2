...
function About_home(dataabouthome) {
  ...
  if (ListaboutHome) {
    ...
    for (let item of dataabouthome) {
         var Comments_posts = "";
        for (let p_comment of item.comments) {
          Comments_posts += `
            <div class="info_comment_video_page">
                <img src="${p_comment.avatar_comment}" alt="">
                <div class="text_logo_name_videos">
                <p>${p_comment.name_user_comment}<i class='bx bxs-check-circle' style='color:#2e88ff'></i></p>
            <p>${p_comment.time_date} ~ <i class='bx bx-world'></i></p>
        </div>
        </div>
            <div class="value_comment_user">
                <p>
