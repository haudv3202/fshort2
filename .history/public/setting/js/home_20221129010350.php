...
function About_home(dataabouthome) {
  ...
  if (ListaboutHome) {
    ...
    for (let item of dataabouthome) {
       var Comments_posts = "";
        for (let posts_comment of item.comments2) {
          Comments_posts += `
                <div class="info_comment_video_page">
            <img src="${posts_comment.avatar_comment2}" alt="">
            <div class="text_logo_name_videos">
                <p>${posts_comment.name_user_comment2}<i class='bx bxs-check-circle' style='color:#2e88ff'></i></p>
                <p>${posts_comment.time_date2} ~ <i class='bx bx-world'></i></p>
            </div>
        </div>
        <div class="value_comment_user">
            <p>
            ${posts_comment.content2}
          </p>
        </div>
        `
        }
